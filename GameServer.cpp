...
//  10054 (WSAECONNRESET) is a hard reset — also treat as drop.
...
static SOCKET waitForReconnect(int expectedSlot)
{
    cout << "[SERVER] Waiting " << RECONNECT_TIMEOUT_SECONDS
        << "s for Player " << expectedSlot << " to reconnect...\n";
...
static bool playOneGame(SOCKET clientSockets[2], vector<Player*>& players)
{
    ...
    while (gameRunning)
    {
        ...
        int column = getValidatedMove(clientSockets[activeIdx],
                                      clientSockets[passiveIdx],
                                      allFields);
        ...
        if (column == -1)
        {
            ...
            char resultCode = (players[passiveIdx]->getId() == 1)
                              ? RESULT_WIN_PLAYER1 : RESULT_WIN_PLAYER2;
    ...
    {
        RawMessage resp;
        if (!safeRecv(clientSockets[i], resp)) continue;
        if (resp.type == MSG_PLAY_AGAIN_YES) yesVotes++;
    }
...
int runGameServer()
{
    ...
    { cerr << "[SERVER] WSAStartup failed." << endl; return 1; }

    SOCKET listenSock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (listenSock == INVALID_SOCKET)
    { cerr << "[SERVER] socket() failed." << endl; WSACleanup(); return 1; }
    ...
    while (keepPlaying)
    {
        ...
        { cerr << "[SERVER] Negotiation failed." << endl; break; }
